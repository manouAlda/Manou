...
const char* asiana_point(const char* tab){
	char* ip_mod = malloc(20);  // La chaîne modifiée peut être plus longue
...
const char* search_broadcast(const char* ip,const char* mask,int* pow_two){
	...
	if(r==0){
		if(cal==1)
			strcat(reseau,"255.255.255");
		else if(cal==2)
			strcat(reseau,"255.255");
		else if(cal==3)
			strcat(reseau,"255");
	}
	else if(r>0){
