...
char* Valiny(int isa){
	...
	isanyNbr=isanJavatra(isa);
		...
		if(isanyNbr==1)
			...
			valiny=Hetsy(isa);
		else if(isanyNbr >=7 && isanyNbr <=10)
			valiny=Tapitrisa(isa);
...
char* Tapitrisa(int isa){
	...
	tap=Separer(isa);
	int tmp=isa%1000000;
	int nbre1=CharTOInt (0,tap);
	int nbr=CharTOInt (1,tap);
	char* let=malloc(1000);
	char* le=malloc(1000);
	char* lettre=malloc(2000);
	
	 if(nbr!=0 && tmp!=0){
	sprintf(let,"%s",Valiny(nbr));
	sprintf(le,"%s",Valiny(nbre1));
	sprintf(lettre,"%s sy %s tapitrisa",le,let);
	}
	else{
	sprintf(lettre,"%s tapitrisa",Valiny(nbr));	
	}
	return lettre;
}
...
char** Separer(int isa){
	...
	for(int j=0,i=strlen(caractere)-6; i<strlen(caractere);i++){
		tap[0][j]=caractere[i];
		j++;
	}
	
	for(int j=0,i=0 ; i<strlen(caractere)-6 ;i++){
		tap[1][j]=caractere[i];
		j++;
	}
