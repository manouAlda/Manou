...
static  void calculate(GtkButton *button,gpointer user_data){
	...
	if(strcmp("C",text)==0){
		gtk_entry_set_text(GTK_ENTRY(box), "");
		memset(input_buffer,0,strlen(input_buffer));
		memset(output_buffer,0,strlen(input_buffer));
...
static void activate (GtkApplication *app,gpointer user_data) {
	...
	gtk_window_set_title(GTK_WINDOW(widget.window),"Les Creations Calculator");
	...
	gtk_grid_attach(GTK_GRID(widget.grid),widget.button[15],0,1,1,1);
